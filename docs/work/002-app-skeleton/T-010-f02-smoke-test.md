...
### 5. Sign-up via email/password

Visit `http://localhost:3000/login` → "Register" → submit email + password. Expected: row appears in `trainers` table. Reset-password test: click "forgot password" → console log shows reset URL (Resend mock mode in local dev).
...
- [ ] Check 5 passed — email/password sign-up + console-logged reset URL
...
- Check 5 in pure local mode: Resend is mocked via the `RESEND_API_KEY.startsWith('test_')` branch in `src/lib/auth.ts`. To test real Resend send, set a valid `RESEND_API_KEY` — but that requires F-03 T-006 Resend domain DKIM verification, so for F-02 the console-log path is sufficient.
