...
- Top-down product sequencing lives in `docs/roadmap.md` (Foundations + Slices with `Unlocks:` / `Prerequisites:` / `Change ID:` fields). Bottom-up status of in-flight initiatives lives in `docs/work/STATUS.md` (auto-regenerated by `~/.claude/scripts/regenerate-status.sh`). The roadmap is hand-edited; STATUS.md is derived — never edit STATUS.md directly.
